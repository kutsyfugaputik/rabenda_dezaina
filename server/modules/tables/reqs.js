...
// Исправлено: Импортируем модель 'Discounts' для работы с таблицей скидок, для указания, применяется ли скидка к запросу.
...
const Requests = sequelize.define('Requests', { 
  ...
  request_id: { 
    type: DataTypes.UUID, 
    // Тип данных для поля 'request_id' — UUID (уникальный идентификатор запроса).
  ...
  feedback_id: { 
    type: DataTypes.UUID,  // Используем UUID, чтобы это поле соответствовало типу в таблице 'feedback'
    allowNull: true, 
  ...
  service_id: { 
    type: DataTypes.INTEGER, 
    // Тип данных для поля 'service_id' — целое число, которое будет ссылаться на услугу.
    allowNull: false, 
  ...
  status_id: { 
    type: DataTypes.INTEGER, 
    // Тип данных для поля 'status_id' — целое число, которое указывает на текущий статус запроса.
    allowNull: false, 
  ...
  client_id: { 
    type: DataTypes.UUID,  // Используем UUID, чтобы это поле соответствовало типу в таблице 'clients'
    allowNull: false, 
    // Указываем, что это поле обязательно для заполнения.
    references: { 
      model: Clients, 
